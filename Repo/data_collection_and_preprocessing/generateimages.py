...
class ImageGenerator():
    ...
    def generate_bills(self, n: int, clazz: int) -> None:
        ...
        sample.to_csv(os.path.join(os.path.join(self.directory, f"{self.s_name}_{str(clazz)}{str(n).zfill(4)}.csv")), sep=';')
    ...
    def _random_basket(self, seed: int) -> pd.DataFrame:
        ...
        threshold = 0.85
        if rnd.normal() > threshold: num_items = rnd.randint(15, 40)
        else: num_items = rnd.randint(1, 15)
...
class SparImageGenerator(ImageGenerator):
    ...
    def _generate_image(self, sample:pd.DataFrame, seed: int) -> (plt.Figure, np.array):
        ...
        for i,row in sample.iterrows():
            ...
            price_and_letter = "{:.2f}".format(row['fullvalue'])+" "+self._random_word(seed=seed+rnd.randint(0,100),
        ...
        num4 = "\t"*3 + self._random_word(seed=seed+rnd.randint(0,100), capital=False, alluper=True, length=(31,33))
...
class PennyImageGenerator(ImageGenerator):
    ...
    def _generate_image(self, sample:pd.DataFrame, seed: int) -> (plt.Figure, np.array):
        ...
        for i,row in sample.iterrows():
            ...
            basket += row['short_title']
            char_in_line = 48 - len(row['short_title']) - 13 - 1  # 11 == spaces and digits after letter, 1 stands for letter
