...
if JOINT_CORNERS_DETECTOR:
    ...
    opt = Adam(params, lr=1e-4)

    train_class.train(optimizer=opt,
                      save_model_path=save_model_path, epochs=150, method=train_class.depict_corners,
                      save_images_path=save_images_path)
if CORNERS_REFINEMENT:
    ...
    for type in ['red', 'green', 'blue', 'yellow']:
        ...
        train_class.train(optimizer=opt,
                          save_model_path=save_model_path, epochs=15, method=train_class.depict_refinement,
                          save_images_path=save_images_path)