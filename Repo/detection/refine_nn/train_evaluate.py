...
class TrainRefine(Train):
    def collate_fn(self, batch):
        ...
        for i, b in enumerate(batch):
            ...
            im = torch.tensor(im).to(torch.float64)
            collated_ims.append(im)

            original = [el / 255 for el in original]
            original = torch.tensor(original).to(torch.float64)
            collated_originals.append(original)

            collated_corners.append(torch.tensor(np.array(corners).flatten()).to(torch.float64))
            collated_corners_big.append(torch.tensor(np.array(corners_big).flatten()).to(torch.float64))
            collated_masks.append(torch.tensor(masks).to(torch.float64))
    ...
    def run_epoch(self, loader: DataLoader, optimizer: torch.optim.Optimizer,
        ...
        for im, _, _, originals, corners_big in tqdm(loader):
            ...
            target_general, offsets = self.initial_crop(originals=originals,
                                                       big_corners_pred=big_corners_pred,
                                                       corners_big=corners_big,
                                                       output_shape=(64, 64))

            # refinement loop
            for j in range(len(target_refine_net)):
                p = patches[j]
                n_p = self.net(p)
                o_p = old_prediction[j]
                t_r_n = target_refine_net[j]
                changes = {}
            ...
            self.writer.add_scalars("Current_epoch_progress",
                                    {
                                        "Loss_of_corners": np.mean(loss_values[-8:]),
                                    },
                                    global_step=i)
            self.writer.flush()
            i += 1
    ...
    def set_JCD(self):
        state_dict = torch.load(os.path.join("progress_tracking", "detection", "corners_nn",
                                             "models", "run_128_3",
                                             "corners_nn_on_ep18_new_best_model_69.0.pt"),
    ...
    def refine_loop(self, optimize, optimizer, criterion, changes, n_p, o_p, t_r_n, p):
        ...
        condition2 = (count / 3) <= 10 if optimize else (count / 3) <= 100
        while condition1 and condition2:
            ...
            t_r_n = t_r_n - torch.tensor([x_start, y_start])
            changes[f"{count:02d} offset"] = torch.tensor([x_start, y_start])
            n_p = n_p - torch.tensor([x_start, y_start])
    ...
    def initial_crop(self, originals: torch.Tensor, big_corners_pred: torch.Tensor,
                     corners_big: torch.Tensor, output_shape=(64, 64), window=70):
        ...
        current_prediction = torch.stack(current_prediction)
        target_refine_net = torch.stack(target_refine_net)
        target_general = torch.stack(target_general)
    ...
    def evaluate(self, method, loader: DataLoader, device: str,
        ...
        for i, b in enumerate(loader):
            ...
            for j in range(len(images)):
                ...
                n_p, changes = self.refine_loop(False, None, torch.nn.MSELoss(reduction="mean"), changes, n_p, o_p, t_r_n, p)
    ...
    def depict_refinement(self, images, fourcorners, singlepredictions, name_convention: str, path: str) -> None:
        ...
        for i, im, four, pred in zip(range(len(images)), images, fourcorners, singlepredictions):
            ...
            corners_pred[(corners_pred > 1000).argwhere()] = 1000
