...
class Train():
    ...
    def run_epoch(self, loader: DataLoader, optimizer: torch.optim.Optimizer,
        ...
        for im, trg in tqdm(loader):
            optimizer.zero_grad()
            if isinstance(self.net, torchvision.models.detection.MaskRCNN):
                # moving to device
            ...
            elif isinstance(self.net, BaseNet):
    ...
    def train(self, optimiser: torch.optim.Optimizer, save_model_path: str,
        ...
        for epoch in range(1, epochs + 1):
            ...
            self.eval.evaluate(method=self.eval.depict, loader=self.train_loader, device=self.device,
                               network=self.net, path=save_images_path, naming=f"train_epoch_{epoch}", writer=self.writer)
...
class Evaluate():
    ...
    def evaluate(self, method, loader: DataLoader, network: torch.nn.Module, device:str,
                 naming:str, path:str, writer:SummaryWriter, num=5):
        ...
        method(images=allimages, masks=allmasks, predictions=allpredictions,
               name_convention=naming, path=path, writer=writer)

    def depict(self, images, masks, predictions, name_convention: str, path: str, writer: SummaryWriter) -> None:
        """
        ...
        for i, image, mask, prediction in zip(range(len(images)), images, masks, predictions):
            ...
            ax[0][2].axis("off")
            ax[1][1].imshow(mask*100, cmap="binary_r")
            ax[1][1].set_title("Actual Mask")
            ax[1][1].axis("off")
            ax[1][2].imshow(prediction*100, cmap="binary_r")
            ax[1][2].set_title("Prediction Mask")
            ax[1][2].axis("off")
            ax[1][0].axis("off")
            fig.savefig(os.path.join(path, name_convention + "_imagenum_" +str(i+1).zfill(2) + ".png"), dpi=600)
