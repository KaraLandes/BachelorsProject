...
if DETECT:
    ...
    bills = CornerRealBillSet(image_dir=os.path.join(repo, "processed_data", "realbills", "unseen"), output_shape=(128, 128))
    ...
    for im, corner_trg, mask_trg in tqdm(loader):
        ...
        for i, image, prediction, target in zip(range(len(im)), im, pred, corner_trg):
            ...
            prediction = [(prediction[0], prediction[1]), (prediction[2], prediction[3]),
